...
.directive('ariIframe', ['$compile', function ($compile) {
  ...
  return {
    ...
    transclude: true,
    scope: {},
    template: '<div class="customiframe"><iframe class="iFrame"></iframe></div>',
    compile: function(iEle, iAttr, transclude) {
      var iframe = $(iEle).find('iframe'),
          doc = iframe[0].contentWindow.document,
          app = iAttr.app;
...
angular.module('fsWebinar', ['ngResource', 'ngAnimate'])
.controller('MailchimpSubscriptionCtrl', ['$log', '$resource', '$scope', '$timeout',
            function ($log, $resource, $scope, $timeout) {
  // Handle clicks on the form submission.
  $scope.addSubscription = function() {
    var mailchimp = $scope.mailchimp;
    ...
    MailChimpSubscription.save(
      ...
      function (response) {
        ...
        if (response.result === 'error') {
          ...
          $timeout(function() {
            $scope.mailchimp.errorMessage = null;
            $scope.mailchimp.result = null;
          }, 5000);
        }
